...
const Filters = props => {
  ...
  const sortBooksByName = () => {
    setFilters({
      ...filters,
      sort_by: { field: "bookname", type: !filters.sort_by.type }
    });
  };
  ...
  const sortBooksByAuthor = () => {
    setFilters({
      ...filters,
      sort_by: { field: "author", type: !filters.sort_by.type }
    });
  };

  return (
    <div className={"filters"}>
      <button className={"myButton"} onClick={sortBooksByName}>
        Sort By Book Name
      </button>

      <button className={"myButton"} onClick={sortBooksByAuthor}>
        Sort By Author Name
      </button>
